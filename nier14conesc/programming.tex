...
example, components cannot be instantiated at run-time. % They are
% rather in-lined at compile time so the compiler can reduce the size of
% the executable binary~\cite{nesc}, which would hardly fit in the
% available program memory otherwise.
...
this example, the \code{report()} function on the line~\lstref{cg:layered}
...
included in this group, specified on the line~\lstref{cg:ctx} after the keyword
\code{contexts} with optional modifiers: \code{is default} to specify the active
context at start-up and \code{is error} to specify an error context on the
line~\lstref{cg:error}. The error context is automatically activated should
...
on the line~\lstref{ct:tr} specifies the allowed outgoing transitions,
...
indicated on the line~\lstref{ct:layer} with the \code{layered} keyword.
Command and event implementations are as in
standard nesC. Particularly, the predefined events \code{activate()}
and \code{deactivate()}, displayed on lines~\lstref{ct:activate}
and~\lstref{ct:deactivate}, are automatically signalled when entering and
...
Additionally, a keyword \code{triggers} is available to bind
...
``Unreachable''. Programmers use \code{triggers} inside \lm{CHECK!}
