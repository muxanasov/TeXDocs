...
this example, the \code{report()} function on line~\lstref{cg:layered}
...
included in this group, specified on line~\lstref{cg:ctx} after the
keyword \code{contexts} with optional modifiers: \code{is default}
...
line~\lstref{ct:tr} specifies the allowed outgoing transitions, wheras
...
\code{activate()} and \code{deactivate()}, shown on
