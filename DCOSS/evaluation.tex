...
Most importantly, the memory overhead is also
negligible, measuring a worst-case 2.5\% penalty for the size of the
program binary and a worst-case 4.5\% penalty for RAM usage. The
characteristics of the applications largely dictate the different
memory overheads. For example, unlike in other cases, in
the wildlife tracking application we need to reserve fairly large
memory buffers to locally store the contact logs.
