...
states, again rendering the implementation necessarily more entangled. The
efforts to make context-unrelated changes in \conesc are the same as in nesC.
...
The context transition overhead arises from the activation rules
(Fig.~\ref{fig:ad}). Several MCU cycles are needed to check is the transition
possible, then system checks the dependencies, and finally, it performs the
instructions in \code{check()}. Additionally, the overhead may vary depending on
the instructions in \code{activated()}/\code{deactivated()}.
