...
\subsection{Evolving the software}\label{sec:evlove}

WSN software needs to constantly evolve due to changes in requirements and
unpredicted environmental dynamics. It is know that the more application is
modularized, the easier is the modification, since the changes in the internal
logic are minor. For each scenario we estimated and compared the efforts
developer needs to spend by modifying the \conesc software against its nesC
conterpart. To this end, we performed three different types of modification:
removing a context, adding a new context, and adding a new context group.
