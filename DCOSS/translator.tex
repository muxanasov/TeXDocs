...
presented by a sample \code{FooGroup} context group and an individual
\code{FooCon} context therein, whereas nesC configurations and modules
with \conesc constructs are represented as \code{BarC} and
\code{BarP}, respectively.
...
context, such as \code{FooGroupBinding} in Fig.~\ref{fig:ctd}. This
module is part of a configuration also automatically generated, such
as \code{FooGroupConfig} in Fig.~\ref{fig:ctd}. Such configuration
uses an automatically generated nesC interface that exports the
layered functions defined in the group, such as \code{FooGroupLayered}
in Fig.~\ref{fig:ctd}. Optionally, an error context is also generated
in plain nesC, as indicated by \code{ErrorFooGroup} in this case, if
...
\code{FooConContext} for the \code{FooGroupConfig} in
Fig.~\ref{fig:ctd}.
