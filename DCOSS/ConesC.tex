...
functions as well as the included contexts. The former is declared on line~\lstref{layereddef}
by using the key-word \emph{layered}, while the declaration of the latter starts from the
key-word \emph{contexts} on line~\lstref{contexts}. It is mandatory to declare
the \emph{default} context, as shown on line~\lstref{isdefault}, which will be
