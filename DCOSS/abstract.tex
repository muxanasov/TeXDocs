\begin{abstract}
  ...
  this issue, we bring concepts from context-oriented programming
  (COP) down to resource-constrained WSN devices. We provide
  \emph{language-independent} design concepts to organize the
  different WSN operating modes as a function of context. Based on
  these, programmers use a notion of \emph{layered function} to
  implement context-dependent behavioral variations of WSN
  code. Separating the design layer from the programming layer
  facilitates porting our solution to different concrete languages.
  Our own implementation, called \conesc, extends nesC
  with COP constructs, and efficiently runs on mainstream WSN
  devices. Based on three representative applications, we show that
  \conesc simplifies the implementations and yields
  increasingly decoupled implementations compared to nesC, easing
  debugging and maintenance. In our tests, this comes at the price of
