...
In order to use context-oriented paradigm in nesC, we added new key-words. We also extended components by \textit{context module} and \textit{context configuration}. These two components have nesC-like structure and utilize the same logic. Both components can be used as other components in native nesC-way.
...
\textbf{Context} component (figure \ref{fig:cc}) represents particular context. It is used to define possible \emph{transitions}, \emph{actions} before activation and after deactivation, \emph{layered functions}, and to \emph{check} transition conditions. There are two types of \emph{checks} for context transitions. At first, we check if the transitions legal, next we check if the context can be activated. The first type of \emph{checks} is declared by the key-word {\textlst{transitions}} on the line \lstref{transitions}. In our example it means that only {\textlst{Normal}} can be activated as the next context after {\textlst{High}}. The definition is not mandatory, but if illegal transition will be called, then {\textlst{Error}} context will be activated. If transitions are not defined, any transitions are legal. Second type of \emph{checks} is method {\textlst{check}} (line \lstref{check}). If triggered transition is legal, we call then function {\textlst{check}}. If it returns {\textlst{FALSE}}, the activation will be ignored and system will remain in the previous state. If method is not defined, conditions will not be verified.

In {\textlst{ConesC}} it is also possible to define a list of instructions, which should be performed during context transition. It is possible by implementing events {\textlst{activated}} and {\textlst{deactivated}}. Event {\textlst{activated}} (line \lstref{activated}) is fired just before context activation, while event {\textlst{deactivated}} (line \lstref{deactivated}) is fired just after deactivation. However, the implementation of these events is not mandatory.
